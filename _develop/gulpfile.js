...
var COPY_FILES = ['css', 'def', 'xml', 'wav', 'mp3', 'mp4', 'json', 'zip', 'inc', 'woff', 'otf', 'pdf', 'map', 'php', 'ico'].join('|');
//-  ここから下は弄らなくてOK
//- ===================================================================  importorts <
...
gulp.task('sass', function() {
  return gulp.src([_src + '**/*.scss']).pipe(plumber({
    errorHandler: function(err) {
      console.log(err.messageFormatted);
      this.emit('end');
    }
  })).pipe(gulpIgnore.exclude('**/_*.scss')).pipe(gulpIgnore.exclude('**/_*/*.scss')).pipe(sass({
    outputStyle: IS_MIN ? 'compact' : 'expanded'
  })).pipe(autoprefixer({
    'browsers': ['last 6 versions', 'IE 8', 'ie 9', 'Firefox >= 2', 'ios 6', 'android 4']
  })).pipe(gulpif(IS_HARDCASE, replace(/  /g, '\t'))).pipe(gulpif(!IS_HARDCASE, replace(/\t/g, '  '))).pipe(gulpif(IS_MIN, replace(/\n\n/g, '\n'))).pipe(gulp.dest(_dest)).pipe(browserSync.stream());
});
//- ===================================================================  html <
gulp.task('pre_html', function() {
  return gulp.src([_src + '**/*.html', '!' + _src + '**/_*.html']).pipe(plumber()).pipe(htmlhint({
    'title-require': false
  })).pipe(htmlhint.reporter()).pipe(gulpif(IS_HARDCASE, replace(/  /g, '\t'))).pipe(gulpif(!IS_HARDCASE, replace(/\t/g, '  '))).pipe(gulp.dest(_dest));
});
gulp.task('rev_html', function() {
  // return gulp.src([_dest + '**/*.html']).pipe(rev({})).pipe(replace(/\.html\?v=.*"/g, '.html"')).pipe(gulp.dest(_dest));
});
...
gulp.task('image', function() {
  return gulp.src([_src + '**/*.+(jpg|jpeg|png|gif|svg)', '!' + _src + '**/_*/**/*.+(jpg|jpeg|png|gif|svg)']).pipe(cache('image'))
    // .pipe(imagemin({
    //   progressive: true,
    //   use: [pngquant({
    //     quality : '60-80',
...
gulp.task('image_sprite', function() {
  ...
  folders.forEach(function(folder) {
    ...
    var spriteData = gulp.src(_source).pipe(spritesmith({
...
gulp.task('js', function() {
  return gulp.src([_src + '**/*.js', '!' + _src + '**/_*.js', '!' + _src + '**/libs.js']).pipe(cache('js')).pipe(plumber({
    errorHandler: function(err) {
      console.log(err.messageFormatted);
      this.emit('end');
    }
  })).pipe(gulpif(IS_MIN, uglify())).pipe(gulp.dest(_dest)).pipe(browserSync.stream());
});
//- ===================================================================  copyFiles <
gulp.task('copy_files', function() {
  return gulp.src([_src + '**/**/*.+(' + COPY_FILES + ')', '!' + _src + '**/_*', '!' + _src + '**/_*/*', _src + '**/libs.js']).pipe(gulp.dest(_dest));
});
//- ===================================================================  clear <
